    ...
    $.post("/symphony/batches/load_batch/", function(data) {}).done(function(data) {
      ...
      $.each(data["batches"], function(index, batch) {
        $("#table-batches").append("<tr>" +
          ...
          "<td>"+ batch["workflow_progress"] + " / " + batch["workflows"].length + "</td>" +
          ...
          (data["is_user_superadmin"] ? "<td><button onclick='deleteBatch(\""+batch["template"]["slug"]+"\""+",\""+batch["id"]+"\")' class='btn btn-sm btn-danger kt-pull-right'>Delete</button></td>" : "") +
        "</tr>");
      });
