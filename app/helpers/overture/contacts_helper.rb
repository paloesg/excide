module Overture::ContactsHelper
  def get_cloned_contact(contact, user)
    ...
    contact.company.present? ? Contact.where(company_name: contact.company_name, cloned_by_id: user.company.id) : Contact.where(company_name: contact.company_name, cloned_by_id: nil)
  end
end
