class Symphony::WorkflowsController < WorkflowsController
  ...
  def xero_create_invoice_payable
    begin
      xero_error = false
      @xero = Xero.new(session[:xero_auth])
      @invoice = @xero.create_invoice_payable(@workflow.workflowable.xero_contact_id, @workflow.invoice.invoice_date, @workflow.invoice.due_date, @workflow.invoice.lineitems, @workflow.invoice.line_amount_types)
