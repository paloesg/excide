class Symphony::BatchesController < ApplicationController
  ...
  def load_batch
    ...
    completed_batches = user_batches.where(completed: true)
    ...
    respond_to do |format|
      format.json  { render json: { user_batches: user_batches.size, batches: @batches, completed_batches: completed_batches.size, is_user_superadmin: (current_user.has_role? :superadmin) } }
    end
  end
