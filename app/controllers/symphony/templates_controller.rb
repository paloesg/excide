class Symphony::TemplatesController < ApplicationController
  ...
  def template_params
    params.require(:template).permit(:title, :company_id, :workflow_type, sections_attributes: [:id, :section_name, :position, tasks_attributes: [:id, :template_id, :position, :task_type, :instructions, :role_id, :document_template_id, :survey_template_id, :days_to_complete, :set_reminder, :important, :link_url, :image_url, :_destroy] ])
  end
end
