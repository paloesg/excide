class NotificationMailer < ApplicationMailer
  ...
  def motif_notify_franchisor(franchisor, user, wfa, link)
    mail(to: franchisor.email, from: 'Asiawide ADA <support@paloe.com.sg>', subject: '', body: 'Some body',  template_id: ENV['ADA_SENDGRID_NOTIFY_FRANCHISOR'], dynamic_template_data: {
  ...
  def motif_renewal_notice_outlet(franchisee, user)
    mail(to: user.email, from: 'Asiawide ADA <support@paloe.com.sg>', subject: '', body: 'Some body',  template_id: ENV['ADA_SENDGRID_FRANCHISEE_RENEWAL_NOTICE'], dynamic_template_data: {
  ...
  def motif_new_outlet(user)
    ...
    mail(to: 'support@paloe.com.sg', from: 'Asiawide ADA <support@paloe.com.sg>', subject: 'New Outlet', body: 'New outlet', template_id: ENV['ADA_SENDGRID_MOTIF_NEW_OUTLET'], dynamic_template_data: {
        fullName: user.full_name,
  ...
  def motif_new_franchisee(user, franchisee_details)
    #to email will be changed with afc's support email
    mail(to: 'jonathan.lau@paloe.com.sg', from: 'Asiawide Digital Advantage (ADA) <admin@excide.co>', subject: 'New Franchisee', body: 'New franchisee', template_id: ENV['ADA_SENDGRID_MOTIF_NEW_FRANCHISEE'], dynamic_template_data: {
        fullName: user.full_name,
        company: user.company.name,
        email: user.email,
