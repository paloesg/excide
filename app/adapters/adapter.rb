module Adapter
  class Xero
    ...
    def create_invoice_payable(contact, date, due_date, item_code, description, quantity, price, account)
      ap = @xero.Invoice.build(type: "ACCPAY", contact: contact, date: date, due_date: due_date)
    ...
    def create_invoice_receivable(type, contact, date, due_date, reference, order, account)
      ar = @xero.Invoice.build(type: type, contact: contact, date: date, due_date: due_date, reference: reference)
